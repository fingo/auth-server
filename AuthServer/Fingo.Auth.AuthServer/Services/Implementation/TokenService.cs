...
namespace Fingo.Auth.AuthServer.Services.Implementation
{
    public class TokenService : ITokenService
    {
        ...
        private readonly IGetUserPolicyConfigurationOrDefaultFromProjectFactory userConfigurationOrDefaultFromProjectFactory;
        ...
        private string UserAuthenticationCheck(string userLogin, string userPassword)
        {
            ...
            {
                _logger.Log(LogLevel.Error, $"HashWithDatabaseSalt((password), {userLogin}) threw exception: {e.Message}.");
                return new JsonObject
                {
                    Result = JsonValues.NotAuthenticated
                }.ToJson();
        ...
        {
            string guidCheckResult = GuidUniquenessCheck(projectGuid);
                    ...
                    foreach (var policyTuple in policyTuples)
                    {
                        switch (policyTuple.Item1)
                        {
                            case Policy.AccountExpirationDate:
                                UserAccountExpirationDateConfiguration userConfiguration =
                                ...
                                if (!Check.AccountExpirationDate((AccountExpirationDateConfiguration) policyTuple.Item2, userConfiguration))
                                    return new JsonObject
                                    {
                            ...
                            case Policy.PasswordExpirationDate:
                                if (!Check.PasswordExpirationDate((PasswordExpirationDateConfiguration) policyTuple.Item2,
            ...
            string userCheckResult = UserAuthenticationCheck(userLogin, userPassword);

            if (userCheckResult != null)
                return userCheckResult;

            string token = EncodePayload(userLogin, projectGuid);
        ...
        public string VerifyToken(string jwt, Guid projectGuid)
        {
            ...
            DecodeResult decodeResult = _jwtLibraryWrapperService.Decode(jwt, JwtConfiguration.SecretKey);
