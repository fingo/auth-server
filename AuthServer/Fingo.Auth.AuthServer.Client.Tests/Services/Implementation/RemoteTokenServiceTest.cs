...
namespace Fingo.Auth.AuthServer.Client.Tests.Services.Implementation
{
    public class RemoteTokenServiceTest
    {
        ...
        public void AcquireTokenShouldThrowServerConnectionException()
        {
            ...
            postServiceMock.Setup(serv => serv.SendAndGetAnswer(It.IsAny<string>(), It.IsAny<Dictionary<string, string>>()))
                .Throws(new Exception());

            IRemoteTokenService remoteTokenService = new RemoteTokenService(postServiceMock.Object);

            // act & assert
            Assert.Throws<ServerConnectionException>(() => remoteTokenService.AcquireToken("sample_login", "sample_password"));
        }
        ...
        public void AcquireTokenShouldReturnNotValidServerAnswerException()
        {
            ...
            // act & assert
            Assert.Throws<ServerNotValidAnswerException>(() => remoteTokenService.AcquireToken("sample_login", "sample_password"));
        }
