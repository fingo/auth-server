...
namespace Fingo.Auth.Domain.CustomData.Factories.Actions.Implementation
{
    public class SaveUserCustomData : ISaveUserCustomData
    {
        ...
        {
            var userCustomData = userCustomDataRepository.FindBy(m => m.ProjectCustomData.ProjectId == projectId && m.UserId == userId && m.ProjectCustomData.ConfigurationName==name).FirstOrDefault() ?? new UserCustomData();
