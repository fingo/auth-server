...
namespace Fingo.Auth.Domain.CustomData.Factories.Actions.Implementation
{
    public class GetCustomDataConfigurationOrDefaultForProject : IGetCustomDataConfigurationOrDefaultForProject
    {
        ...
        public CustomDataConfiguration Invoke(int projectId, string configurationName, ConfigurationType configurationType)
        {
            ...
            if (string.IsNullOrEmpty(configurationName))
            {
                switch (configurationType)
                {
                    ...
                    default:
                        throw new Exception($"Wrong policy ({configurationType}) given to GetPolicyConfigurationOrDefaultFromProject.");
                }
            }
            ...
            if (projectCustomData != null)
            {
                try
                {
                    return jsonConvertService.DeserializeProject(configurationType, projectCustomData.SerializedConfiguration);
                }
                catch (Exception e)
                {
                    throw new Exception(
