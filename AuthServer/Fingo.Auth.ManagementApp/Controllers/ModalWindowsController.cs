...
namespace Fingo.Auth.ManagementApp.Controllers
{
    ...
    public class ModalWindowsController : Controller
    {
        ...
        public ModalWindowsController(IGetProjectsFromUserFactory getProjectsFromUserFactory, IGetUserFactory getUserFactory,
        ...
        {
            return PartialView("User/ProjectConnectedWithUserCollapseWindow", getProjectsFromUserFactory.Create().Invoke(userId));
        }
        ...
        public PartialViewResult UserModalWindow(ModalWidows modalOption, int projectId, int userId)
        {
            ...
            switch (modalOption)
            {
                ...
                case ModalWidows.AssignUsers:
                    return PartialView("User/AssignUsersModalWindow", allNotAssignedUsersToProjectFactory.Create().Invoke(projectId));
        ...
        public PartialViewResult PolicyModalWindow(int projectId, Crud crudOption, Policy policy)
        {
            ...
            {
                case Crud.Add:
                    return PartialView("Policy/AddPolicyModalWindow", getPoliciesUnassignedToProjectFactory.Create().Invoke(projectId));
                case Crud.Edit:
        ...
        public PartialViewResult ProjectModalWindow(ModalWidows modalOptions, int userId, int projectId)
        {
            ...
            switch (modalOptions)
            {
                case ModalWidows.UnassignUser:
                    return PartialView("Project/UnassignFromProjectModalWindow", getProjectFactory.Create().Invoke(projectId));
            }
            return new PartialViewResult();
        }

        [HttpGet("customData")]
        public PartialViewResult CustomDataModalWindow(int projectId, Crud crudOption, string configurationName, ConfigurationType configurationType)
        {
            ViewBag.ProjectId = projectId;
            ...
            {
                case Crud.Add:
                    return PartialView("CustomData/AddCustomData", Enum.GetValues(typeof(ConfigurationType)).OfType<ConfigurationType>());
                case Crud.Edit:
        ...
        public PartialViewResult UserCustomDataModalWindow(int projectId, int userId)
        {
            ViewBag.ProjectId = projectId;
            ViewBag.UserId = userId;
            return PartialView("CustomData/EditUserCustomData", getProjectWithCustomDataFactory.Create().Invoke(projectId));
        }
    }
}
