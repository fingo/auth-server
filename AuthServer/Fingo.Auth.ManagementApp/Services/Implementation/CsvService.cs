...
using Fingo.Auth.Domain.Infrastructure.EventBus.Events.Base.Export;
...
namespace Fingo.Auth.ManagementApp.Services.Implementation
{
    public class CsvService : ICsvService
    {
        ...
        public string ExportProject(int projectId)
        {
            ...
            var result = "Project name;Project GUID;Client name;Client contact data;Creation date UTC;Modification date UTC\r\n";
            result += $"{ToCsvString(project.Name)};{ToCsvString(project.ProjectGuid.ToString())};{ToCsvString(project.Information.Name)};" +
            ...
            using (var stream = csvFile.OpenReadStream())
            {
                using (var reader = new StreamReader(stream))
                {
                    while (!reader.EndOfStream)
                    {
                        ...
                        if (values.Length != 3)
                            throw new FormatException("Each line must contain three and only three tab characters.");
