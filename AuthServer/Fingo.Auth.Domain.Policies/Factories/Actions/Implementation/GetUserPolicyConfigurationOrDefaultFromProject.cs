...
namespace Fingo.Auth.Domain.Policies.Factories.Actions.Implementation
{
    public class GetUserPolicyConfigurationOrDefaultFromProject: IGetUserPolicyConfigurationOrDefaultFromProject
    {
        ...
        public PolicyConfiguration Invoke(int userId, Policy policy, int projectId)
        {
            ...
            var userPolicy =
                user.UserPolicies
                    .FirstOrDefault(m => m.ProjectPolicies.Policy == policy && m.ProjectPolicies.ProjectId == projectId);
            if (userPolicy != null)
            {
                try
                {
            ...
            switch (policy)
            {
                ...
                default:
                    throw new Exception($"Wrong policy ({policy}) given to GetUserPolicyConfigurationOrDefaultFromProject.");
            }
        }
    }
}