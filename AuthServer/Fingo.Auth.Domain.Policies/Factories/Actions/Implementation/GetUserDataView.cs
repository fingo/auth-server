...
namespace Fingo.Auth.Domain.Policies.Factories.Actions.Implementation
{
    public class GetUserDataView : IGetUserDataView
    {
        ...
        public UserDateView Invoke(int projectId, int userId)
        {
            ...
            var userPolicy = user.UserPolicies.FirstOrDefault(m=>m.ProjectPolicies.ProjectId==projectId && m.ProjectPolicies.Policy==Policy.AccountExpirationDate);

            if (userPolicy != null)
            {
                UserAccountExpirationDateConfiguration result =
