...
namespace Fingo.Auth.Domain.Users.Implementation
{
    public class AddUser : IAddUser
    {
        ...
        public AddUser(IUserRepository repository, IProjectRepository projectRepository, IEventBus eventBus, ISetDefaultUserCustomDataBasedOnProject setDefaultUserCustomDataBasedOnProject)
        {
        ...
        {
            Project project = _projectRepository.GetByGuid(guid).WithoutStatuses(ProjectStatus.Deleted);

            if (project == null)
                throw new ArgumentNullException($"Cannot add user to project with Guid:{guid}, because this project has been deleted.");
